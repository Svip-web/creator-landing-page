...
function renderHero(ls){
  ...
  const description=esc(ls[5])
    .replace('От точки, в которой ваш бизнес находится сейчас','<strong>От точки, в которой ваш бизнес находится сейчас</strong>')
    .replace('растёт системно','<strong>растёт системно</strong>')
    .replace('сильной командой','<strong>сильной командой</strong>')
    .replace('выстроенной финансовой моделью','<strong>выстроенной финансовой моделью</strong>')
    .replace('чётким планом развития','<strong>чётким планом развития</strong>');
return '<section class="document-section block-1 figma-hero" id="block-1" data-block="1"><div class="figma-hero-bg" aria-hidden="true"><video class="hero-stage-video" autoplay muted loop playsinline preload="metadata" poster="assets/hero-stage-screen-audience.webp"><source src="assets/hero-stage-loop.mp4" type="video/mp4"></video></div><div class="figma-hero-people" aria-hidden="true"><img class="figma-person figma-lera" src="assets/lera-cutout.webp" alt="" width="1024" height="1536" fetchpriority="high" decoding="async"><img class="figma-person figma-tikhon" src="assets/tikhon-cutout.webp" alt="" width="1024" height="1536" fetchpriority="high" decoding="async"></div><div class="document-copy figma-hero-copy"><div class="figma-meta">'+renderLine(ls[0],0)+renderLine(ls[1],1)+'</div><h1>'+esc(ls[2])+'</h1><div class="figma-subtitle"><h2><span>'+subtitle.map(esc).join('</span><span>')+'</span></h2></div><p class="figma-byline">'+esc(ls[4])+'</p><p class="figma-description">'+description+'</p>'+renderLine(ls[7],7)+'</div></section>';
}
...
function layout(n,ls){
  ...
  if(n===6){
    ...
    const finalCopy=esc(ls[5]).replace('с двух сторон','<strong>с двух сторон</strong>').replace('собственную работающую систему','<strong>собственную работающую систему</strong>');
    return '<div class="collab-layout"><div class="collab-intro"><h2>'+esc(ls[0])+'</h2><h3>'+tagline+'</h3></div><div class="collab-stage" aria-hidden="true"><div class="collab-half collab-europe"><i></i><img src="assets/lera-cutout.webp" alt="" loading="lazy" decoding="async"><span class="collab-name">ЛЕРА РУМА</span></div><div class="collab-symbol"><span>×</span></div><div class="collab-half collab-ukraine"><i></i><img src="assets/tikhon-cutout.webp" alt="" loading="lazy" decoding="async"><span class="collab-name">ТИХОН БЕЛЯЕВ</span></div></div><div class="collab-story"><p>'+esc(ls[2])+'</p><p>'+esc(ls[3])+'</p></div><div class="collab-conclusion"><p class="collab-power">'+esc(ls[4])+'</p><p class="collab-final">'+finalCopy+'</p></div></div>';
  }
  if(n===8){
  ...
  if(n===17){
    const emphasis=[
      [['Сильный бьюти-бизнес'],['хорошей услуге, сильном мастере или удачной рекламе']],
      [['системы'],['работают на развитие бизнеса']],
      [['практическая программа'],['на своём проекте'],['что именно нужно менять']]
    ];
    const body=ls.slice(2,5).map((line,i)=>{
      let copy=esc(line);
      emphasis[i].flat().forEach(part=>copy=copy.replace(part,'<strong>'+part+'</strong>'));
      return '<p>'+copy+'</p>';
    }).join('');
    return renderLine(ls[0],0)+renderLine(ls[1],1)+body+ls.slice(5).map((line,i)=>renderLine(line,i+5)).join('');
  }
