...
return '<section class="document-section block-1 figma-hero" id="block-1" data-block="1"><div class="figma-hero-bg" aria-hidden="true"><video class="hero-stage-video" autoplay muted loop playsinline preload="auto" poster="assets/hero-stage-rich-v2.png"><source src="assets/hero-stage-loop.mp4" type="video/mp4"></video></div><div class="figma-hero-people" aria-hidden="true"><img class="figma-person figma-lera" src="assets/lera-cutout.png" alt=""><img class="figma-person figma-tikhon" src="assets/tikhon-cutout.png" alt=""></div><div class="document-copy figma-hero-copy"><div class="figma-meta">'+renderLine(ls[0],0)+renderLine(ls[1],1)+'</div><h1>'+esc(ls[2])+'</h1><div class="figma-subtitle"><h2><span>'+subtitle.map(esc).join('</span><span>')+'</span></h2></div><p class="figma-byline">'+esc(ls[4])+'</p><p class="figma-description">'+esc(ls[5])+'</p>'+renderLine(ls[7],7)+'</div></section>';
}
...
function layout(n,ls){
  ...
  if(n===14){
    ...
    const tariffPhotos=['tariff-creator-v2.png','tariff-pro-v2.png','tariff-vip-v2.png'];
    return ls.slice(0,starts[0]).map(renderLine).join('')+'<div class="participation-grid">'+starts.map((s,i)=>{const part=ls.slice(s,starts[i+1]??end);return '<article class="participation-card tariff-'+(i+1)+'"><figure class="tariff-photo"><img src="assets/'+tariffPhotos[i]+'" alt="" loading="lazy"><h3 class="participation-name">'+esc(part[0])+'</h3></figure><div class="participation-card-body"><details class="participation-details" open><summary>'+esc(part[1])+'</summary><div>'+part.slice(2).map((l,j)=>renderLine(l,j+2)).join('')+'</div></details></div></article>'}).join('')+'</div><div class="participation-help">'+ls.slice(end).map((l,i)=>renderLine(l,i+1)).join('')+'</div>';
  }
  if(n===7){
    const split=ls.indexOf('ТИХОН БЕЛЯЕВ');
    return [ls.slice(0,split),ls.slice(split)].map((part,i)=>{const firstBullet=part.findIndex(x=>x.startsWith('—'));const closing=part.at(-1);const intro=part.slice(2,firstBullet);const bullets=part.slice(firstBullet,-1);return '<article class="mentor-story mentor-'+(i?'tikhon':'lera')+'"><div class="mentor-stage" aria-hidden="true"><i></i><img loading="lazy" src="assets/'+(i?'tikhon-cutout.png':'lera-cutout.png')+'" alt=""></div><div class="mentor-content"><header><h2>'+esc(part[0])+'</h2><h3>'+esc(part[1])+'</h3></header><div class="mentor-intro">'+intro.map((line,j)=>'<p class="'+(j===intro.length-1?'mentor-list-label':'')+'">'+esc(line)+'</p>').join('')+'</div><div class="mentor-experience">'+bullets.map((line,j)=>renderLine(line,j)).join('')+'</div><p class="mentor-final">'+esc(closing)+'</p></div></article>'}).join('');
  }
  if(n===9){
    ...
    const mockupPhotos=['methodology-generated.webp','work-table.png','lera-live-event.png','beauty-studio.png','personal-strategy-generated.webp','lera-stage-talk.png','hero-duo-extended.jpg','two-markets-generated.webp'];
    ...
    const cards=modules.map((module,i)=>{
      ...
      return '<details class="module module-product"><summary><span class="module-object object-'+(i+1)+'" data-index="'+String(i+1).padStart(2,'0')+'" data-label="'+mockupLabels[i]+'" aria-hidden="true"><i class="mockup-back"></i><i class="mockup-face"><img src="assets/'+mockupPhotos[i]+'" alt="" loading="lazy"><b></b></i><i class="mockup-edge"></i></span><span class="module-title">'+esc(module[0])+'</span><span class="module-plus" aria-hidden="true">+</span></summary><div class="module-body">'+body+'</div></details>';
    }).join('');
  ...
  if(n===3){
    ...
    const chatPhotos=['methodology-generated.webp','two-markets-generated.webp','live-work-generated.webp','personal-strategy-generated.webp','lera-stage-talk.png','work-table.png','speaker-finance-event.jpg','beauty-studio.png','lera-event-audience.png'].map((src,i)=>'<figure class="chat-photo chat-photo-'+(i+1)+'"><img src="assets/'+src+'" alt="" loading="lazy"></figure>').join('');
  ...
  if(n===4){
    const taskPhotos=['speaker-finance-event.jpg','work-table.png','beauty-studio.png','lera-live-event.png','hero-duo-extended.jpg','lera-event-audience.png'];
    ...
    const slides=groups.map((task,i)=>'<article class="task-slide" data-slide="'+i+'"><figure class="task-photo"><img src="assets/'+taskPhotos[i]+'" alt="" loading="lazy"></figure><div class="task-copy"><h2>'+esc(task[0])+'</h2><h3>'+esc(task[1])+'</h3><div class="task-action"><p>'+esc(task[2])+'</p><p>'+esc(task[3])+'</p></div><div class="task-result"><p>'+esc(task[4])+'</p><p>'+esc(task[5])+'</p></div></div></article>').join('');
  ...
  if(n===6){
    ...
    return '<div class="collab-layout"><div class="collab-intro"><h2>'+esc(ls[0])+'</h2><h3>'+tagline+'</h3></div><div class="collab-stage" aria-hidden="true"><div class="collab-half collab-europe"><i></i><img src="assets/lera-cutout.png" alt=""></div><div class="collab-symbol"><span>×</span></div><div class="collab-half collab-ukraine"><i></i><img src="assets/tikhon-cutout.png" alt=""></div></div><div class="collab-story"><p>'+esc(ls[2])+'</p><p>'+esc(ls[3])+'</p></div><div class="collab-conclusion"><p class="collab-power">'+esc(ls[4])+'</p><p class="collab-final">'+esc(ls[5])+'</p></div></div>';
  }
  if(n===8){
    const photos=['beauty-studio.png','work-table.png','lera-stage-talk.png','personal-strategy-generated.webp'];
    const accents=[
    ...
    const cards=[0,1,2,3].map((_,i)=>{
      ...
      return '<article class="audience-card"><figure><img src="assets/'+photos[i]+'" alt="" loading="lazy"></figure><div><h2>'+title+'</h2><p>'+copy+'</p></div></article>';
    }).join('');
...
function renderAbout(ls){
  ...
  for(let i=0;i<features.length;i+=2){let copy=esc(features[i+1]);for(const phrase of featureHighlights[i/2])copy=copy.replace(phrase,'<strong>'+phrase+'</strong>');cards+='<article class="about-feature"><figure class="about-feature-photo feature-photo-'+(i/2)+'"><img src="assets/'+featurePhotos[i/2]+'" alt="" loading="lazy" width="600" height="450"></figure><h3>'+esc(features[i])+'</h3><p>'+copy+'</p></article>'}
